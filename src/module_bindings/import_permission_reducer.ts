...
export class ImportPermissionReducer extends Reducer
{
	...
	public static call(_identity: Uint8Array, _permissionLevel: string) {
		this.getReducer().call(_identity, _permissionLevel);
	}

	public call(_identity: Uint8Array, _permissionLevel: string) {
		const serializer = this.client.getSerializer();
		let _identityType = AlgebraicType.createArrayType(AlgebraicType.createPrimitiveType(BuiltinType.Type.U8));
	...
	public static deserializeArgs(adapter: ReducerArgsAdapter): any[] {
		let identityType = AlgebraicType.createArrayType(AlgebraicType.createPrimitiveType(BuiltinType.Type.U8));
		let identityValue = AlgebraicValue.deserialize(identityType, adapter.next())
		let identity = identityValue.asBytes();
	...
	public static on(callback: (reducerEvent: ReducerEvent, _identity: Uint8Array, _permissionLevel: string) => void) {
		this.getReducer().on(callback);
	}
	public on(callback: (reducerEvent: ReducerEvent, _identity: Uint8Array, _permissionLevel: string) => void)
	{
