  ...
  return (
    <Selecto
      ...
      onDragStart={(e) => {
        ...
        if (moveable.isMoveableElement(target) || selectoTargets.some((t) => t === target || t.contains(target))) {
          e.stop();
        }
      }}
      onSelectEnd={(e) => {
        if (e.selected.length === 0 && selectoTargets.length === 0) return;
        ...
        if (e.selected.length === 1) {
          const selectedElement = elements.filter((element: Elements) => element.id === parseInt(e.selected[0].id));
        ...
        setSelectoTargets(unlockedElements);
      }}
    />
  );
};
