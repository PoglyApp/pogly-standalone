...
export const SettingsModal = (props: IProp) => {
	...
	const [cursorNameCheckbox, setCursorNameCheckbox] = useState<boolean>(
	...
	const showInstancePassword = () => {
		setModals((oldModals: any) => [
			...oldModals,
			<InstancePasswordModal key='instancePassword_modal' />,
		]);
	};
	...
	const showUploadModal = () => {
		setModals((oldModals: any) => [
			...oldModals,
			<BackupModal key='backup_modal' download={false} />,
		]);
	};
	...
	const showDownloadModal = () => {
		setModals((oldModals: any) => [
			...oldModals,
			<BackupModal key='backup_modal' download={true} />,
		]);
	};
				...
				<FormGroup sx={{ gap: '10px' }}>
					...
					<div style={{ display: 'grid' }}>
						...
						<Typography
							...
							color='#ffffffa6'
							onClick={() =>
								openInNewTab('https://developers.google.com/tenor/guides/quickstart#setup')
							}
							sx={{
								'&:hover': {
									cursor: 'pointer',
					...
					{props.onlineVersion !== `${import.meta.env.VITE_APP_VERSION}` && (
						<Alert
							variant='filled'
							severity='warning'
							sx={{
						...
						>
							...
							<a href='https://github.com/PoglyApp/pogly-standalone/releases'>
